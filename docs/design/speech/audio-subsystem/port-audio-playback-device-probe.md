...
**Purpose**: Enumerate playback-capable devices from the current platform's preferred PortAudio
host API.
...
- **Enumerate()**: Resolves the preferred host API, filters the PortAudio device catalog to
  devices on that host API with `MaxOutputChannels > 0`, and maps each result to
...
**Callers**: `AudioDeviceFactory` and hosts reading `AudioDeviceFactory.PlaybackProbe`.

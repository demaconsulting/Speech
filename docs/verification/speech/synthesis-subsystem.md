...
decimal point such as `0.5`) is never treated as a boundary, so numerals stay intact, while the
same punctuation still splits normally when only one side is a digit. Also verifies that a
